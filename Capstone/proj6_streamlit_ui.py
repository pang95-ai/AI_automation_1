...
def load_api_key(env_file: str | None = None) -> str:
    ...
    On HuggingFace Spaces we write a temporary ``OpenAI_key.env`` from the
    `OPENAI_API_KEY` secret, but the key may also simply be present in the
    environment.  ``env_file`` is optional and only loaded if provided.
    """
